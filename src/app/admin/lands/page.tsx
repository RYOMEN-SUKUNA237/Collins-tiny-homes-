      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-sm border-b border-sage/10 px-8 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-charcoal-light text-xs mt-0.5">
            {lands.length} parcel{lands.length !== 1 ? "s" : ""} listed
          </p>
        </div>
        <Link
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left border-b border-sage/10 bg-offwhite/50">
                  <th className="px-7 py-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">
                    Parcel / Location
                  </th>
                  <th className="px-4 py-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">
                    Type & Price
                  </th>
                  <th className="px-4 py-4 text-xs font-semibold text-charcoal-light uppercase tracking-wider">
                    Details
                ...
                {lands.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-7 py-12 text-center">
                      ...
                      <Link
                        href="/admin/lands/new"
                        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-sage text-white text-sm font-semibold"
                      >
                  ...
                  lands.map((land) => (
                    <tr
                      key={land.id}
                      className="hover:bg-sage/3 transition-colors"
                    >
                      ...
                      <td className="px-4 py-4">
                        <span
                          className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide mb-1 ${land.price_type === "rent" ? "bg-clay/15 text-clay-dark" : "bg-sage/15 text-sage-dark"}`}
                        >
                          {land.price_type}
                        </span>
                        <p className="font-semibold text-charcoal text-sm">
                      ...
                      <td className="px-7 py-4">
                        <div className="flex items-center gap-2">
                          <Link
                            href={`/admin/lands/${land.id}/edit`}
                            className="p-2 rounded-lg text-charcoal-light hover:text-clay hover:bg-clay/10 transition-colors"
                            title="Edit"
                          >
