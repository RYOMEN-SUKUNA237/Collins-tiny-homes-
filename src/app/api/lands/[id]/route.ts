  ...
  try {
    ...
    return NextResponse.json({
      ...
      utilities: land.utilities
        ? typeof land.utilities === "string"
          ? JSON.parse(land.utilities)
          : land.utilities
        : { water: false, electric: false, sewage: false },
      images: Array.from(
        new Set([land.cover_image, ...dbImages.map((i) => i.url)]),
      ),
      gallery: dbImages,
...
export async function PUT(
  ...
  try {
    ...
    if (data.utilities !== undefined)
      dbData.utilities = JSON.stringify(data.utilities);
